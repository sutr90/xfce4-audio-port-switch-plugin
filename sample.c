                                 ...
                                 SamplePlugin *sample)
{
    ...
    panel_size   = xfce_panel_plugin_get_size (sample->plugin);
    panel_size  /= xfce_panel_plugin_get_nrows (sample->plugin);

    style = gtk_widget_get_style (GTK_WIDGET (sample->btn));
    fgcolor = style->fg[GTK_STATE_NORMAL];
    group_name = sample->filename;

    cr = gdk_cairo_create ((GTK_WIDGET (sample->layout_image))->window);
